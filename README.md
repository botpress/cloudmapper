...
1. To view the interactive network diagram generated by CloudMapper, open:

   http://localhost:8000/

   You can extract the files for the generated diagram directly from the local `web/` folder in this repository or the raw collected AWS account data from `account-data/`, as these folders are mounted as host volumes by the `start.sh` script so they're written to directly by the container.
