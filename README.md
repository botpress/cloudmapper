> This is a fork of the original [CloudMapper](https://github.com/duo-labs/cloudmapper) repository with customizations to make it easy to generate an up-to-date network diagram of the Botpress Cloud production environment in AWS.

## Usage

1. Clone this repository.
1. Copy the `.env.example` file to a new `.env` file.
1. Go to https://botpress-aws.awsapps.com/start, select the AWS account you want to generate the diagram for, and then click on the **"Command line or programmatic access"** link besides the **CloudMapper** role.
1. Copy the Access Key ID, Secret Access Key, and Session Token values displayed and paste them in the corresponding places of the `.env` file.
   > Note: Make sure the `ACCOUNT` variable in the `.env` file is set to the `name` value found in the `config.json` file that corresponds to the AWS account you're generating the diagram for. This file contains the list of account IDs and names that CloudMapper can use, and can always be modified to include other AWS accounts as well.
...
1. Open the interactive network diagram generated by CloudMapper:

   http://localhost:8888/

   You can extract the files for the generated diagram directly from the local `web/` folder in this repository or the raw collected AWS account data from `account-data/`, as these folders are mounted as host volumes by the `run.sh` script so they're written to directly by the container.
...
Cloudmapper needs to make IAM calls and cannot use session credentials for collection, so you cannot use the aws-vault server if you want to collect data, and must pass role credentials in directly or configure aws credentials manually inside the container. _The following code exposes your raw credentials inside the container._
